...
export default class ReactUserTour extends Component {
    ...
    getCustomArrow(position) {
        return typeof this.props.arrow === 'function'
            ? this.props.arrow({
    ...
    render() {
        ...
        let width = currentTourStep.width || this.width;
        let height = currentTourStep.height || this.height;

        if (typeof width === 'string' && width.indexOf('%') >= 0) {
            width = window.innerWidth * (width.replace('%', '') / 100);
        }

        if (typeof height === 'string' && height.indexOf('%') >= 0) {
            height = window.innerHeight * (height.replace('%', '') / 100);
        }
        ...
        const tourStepsCounter = !this.props.hideSteps ? (
            <div
                style={{
                    position: 'absolute',
                    left: 20,
                    bottom: 10,
                    ...this.props.tourStepsCounterStyle
                }}
            >
        ...
        const tourButtonContainer = !this.props.hideButtons ? (
        ...
        const closeButton = !this.props.hideClose ? (
        ...
        const maskPosition = this.getMaskPositionAndDimensions({
            selector: currentTourStep.selector
        });
